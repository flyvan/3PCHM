...
double WeightedGaussTransform(const double* A, const double* B, double* WeightA, double* WeightB, 
	int m, int n, int dim, double scale, double* grad)
{
	double cross_term = 0;
    for (int i = 0; i < m * dim; ++i) 
	...
	for (int i = 0; i < m; ++i)
	{
		...
		for (int d = 0; d < dim; ++d)
		{
			dist_ii +=  SQR(A[i * dim + d] - B[i * dim + d]);
 		}
		double cost_ii = WeightA[i] * WeightB[i] * exp(-1.0 * dist_ii / scale);
	    for (int d = 0; d < dim; ++d)
		{
			grad[i * dim + d] -= cost_ii * 2.0 * (A[i * dim + d] - B[i * dim + d]);
	    }
		cross_term += cost_ii;
...
double WeightedGaussTransform(const vnl_matrix<double>& A, const vnl_matrix<double>& B, double* WeightA, double* WeightB, 
	double scale, vnl_matrix<double>& gradient) 
{
  // assert A.cols() == B.cols()
  return WeightedGaussTransform(A.data_block(), B.data_block(), WeightA, WeightB,
