...
void gmmreg_rigid::start_registration(vnl_vector<double>& params, int n_max_func_evals) {
...
void gmmreg_rigid::set_init_params() {
  set_init_rigid();
}

void gmmreg_rigid::set_init_rigid() {
  assert((d == 2) || (d == 3));
  if (d == 2) {
...
void gmmreg_rigid::perform_transform(const vnl_vector<double> &x) {
  ...
  param_rigid = x;
  cout<<"The "<< ++iterid <<"th iter Rigid" <<endl;
}
...
void gmmreg_rigid::prepare_own_options() {
